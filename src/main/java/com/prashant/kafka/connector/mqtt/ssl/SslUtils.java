package com.prashant.kafka.connector.mqtt.ssl;
...
public class SslUtils {
	 ...
	 */
	public static SSLSocketFactory getSslSocketFactory(String caCrt, String crt, String key, String password)
			throws IOException, KeyStoreException, NoSuchAlgorithmException, CertificateException,
			UnrecoverableKeyException, KeyManagementException {

		char[] passwdChars = password != null && password.length() > 0 ? password.toCharArray() : "".toCharArray();

		// load client private key
		PEMParser parser = new PEMParser(new InputStreamReader(new FileInputStream(key), Charset.forName("UTF-8")));
		...
		// load CA certificate
		parser = new PEMParser(new InputStreamReader(new FileInputStream(caCrt), Charset.forName("UTF-8")));
		...
		parser = new PEMParser(new InputStreamReader(new FileInputStream(crt), Charset.forName("UTF-8")));
